...
export default function WithdrawListPage() {
  ...
  async function fetchAll() {
    ...
    try {
      const [pending, approved, rejected] = await Promise.all([
        ...
        getWithdrawalsByStatus('approved'),
        getWithdrawalsByStatus('rejected'),
      ]);
        ...
        <table style={styles.table}>
          <thead style={styles.thead}>
            <tr>
              ...
              <th style={styles.th}>用户ID</th>
              <th style={styles.th}>提现数量 (Pi)</th>
          ...
          <tbody>
            {data.map((item, i) => (
              ...
              >
                ...
                <td style={styles.td}>{item.accountId || '—'}</td>
                <td style={styles.td}>
                  <span
                    style={{
                      ...styles.status,
  ...
  return (
    <main style={styles.page}>
      <h1 style={styles.pageTitle}>提现审核列表</h1>
      {renderTable('待审核提现申请', pendingList, true)}
      {renderTable('提现成功记录', approvedList, false)}
      {renderTable('提现失败记录', rejectedList, false)}
    </main>
  );
}
